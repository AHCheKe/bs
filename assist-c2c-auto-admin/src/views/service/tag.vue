<template>
    <div>
      ...
      <el-table v-loading="loading" :data="list">
        ...
        <el-table-column label="操作" align="center">
          <template #default="scope">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row.tagId)">修改</el-button>
            <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row.tagId)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  const getList = () => {
    loading.value = true;
    baseService
      .get("/admin/service/tag/list", queryForm)
      .then((res) => {
        loading.value = false;
        if (res.code === 200) {
  ...
  function handleUpdate(id: string) {
    ...
    title.value = "修改";
    reset();
    baseService.get(`/admin/service/tag/info/${id}`).then((res) => {
      if (res.code === 200) {
        form.value = res.data;
      }
    });
  ...
  function submitForm() {
    formRef.value.validate((valid: boolean) => {
      if (!valid) return;
      baseService.post(`/admin/service/tag/save`, form.value).then((res) => {
        if (res.code === 200) {
  ...
  function handleDelete(id: any) {
    ElMessageBox.confirm("确认要删除当前项吗?", "提示").then(() => {
      baseService.get(`/admin/service/tag/delete/${id}`, id).then((res) => {
        if (res.code === 200) {
          ElMessage.success(res.msg);
          getList();
        } else {
