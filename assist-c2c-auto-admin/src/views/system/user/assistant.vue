    ...
    <div>
      ...
      <el-table v-loading="loading" :data="list">
        ...
        <el-table-column label="标签" align="center" prop="tags" min-width="120">
          <template #default="scope">
            <el-tag type="primary" v-for="(item, index) in scope.row.userTagList" :key="index" style="margin-right: 5px;">{{ item.tag.tagName }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="评分" align="center" prop="rate" width="180">
          <template #default="scope">
            <el-rate
              v-model="scope.row.rate"
              allow-half size="small"
              disabled
            />
        ...
        <el-table-column label="状态" align="center" prop="status" width="80">
          <template #default="scope">
            ...
            <!-- <el-tag type="warning" v-if="scope.row.status==2">审核失败</el-tag> -->
            <el-tag type="info" v-if="scope.row.status==-1">忙碌</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" min-width="150">
          <template #default="scope">
            <el-button link type="success" icon="Unlock" @click="handleAuth(scope.row.userId)" v-if="scope.row.status>0">设为授权用户</el-button>
            <div>
              <el-button link type="primary" icon="Document" @click="showDetail(scope.row)">详情</el-button>
              <!--<el-button link type="danger" icon="Close" @click="handleDelete(scope.row.userId)" v-if="scope.row.status==1">禁用</el-button>
      ...
      <el-dialog v-model="openDetail" :title="title" width="60%" append-to-body>
        <el-descriptions class="margin-top" :column="1" border v-if="detailData">
          ...
          <el-descriptions-item label="审核时间" v-if="detailData.certify.certifyStatus>1">
