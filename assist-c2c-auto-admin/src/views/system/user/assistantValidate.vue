<template>
    <div>
      ...
      <el-table v-loading="loading" :data="list">
        ...
        <el-table-column label="审核状态" align="center" prop="role" width="120">
          <template #default="scope">
            <el-tag type="warning" v-if="scope.row.certify.certifyStatus==0">待审核</el-tag>
            <el-tag type="success" v-if="scope.row.certify.certifyStatus==1">审核通过</el-tag>
            <el-tag type="info" v-if="scope.row.certify.certifyStatus==2">已拒绝</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="250">
          <template #default="scope">
            <el-button link type="primary" icon="Document" @click="showDetail(scope.row)">详情</el-button>
            <el-button link type="success" icon="Check" @click="handelCheck(scope.row.userId)" v-if="scope.row.certify.certifyStatus==0">通过</el-button>
            <el-button link type="danger" icon="Close" @click="handelCheck2(scope.row.userId)" v-if="scope.row.certify.certifyStatus==0">拒绝</el-button>
            <el-button link type="success" icon="Check" @click="handelCheck(scope.row.userId)" v-if="scope.row.certify.certifyStatus==2">重新审核</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  <script setup lang="ts">
    ...
    const getList = () => {
      loading.value = true;
      baseService
        .post("/admin/user/list", queryForm)
        .then((res) => {
          loading.value = false;
          if (res.code === 200) {
    ...
    function handelCheck(id: string) {
      ElMessageBox.confirm("确认通过该陪诊师的申请吗?", "提示").then(() => {
        baseService.get(`/admin/user/assist/validate/${id}`).then((res) => {
          if (res.code === 200) {
            getList();
            ElMessage({
              message: '已通过陪诊师申请',
    ...
    function handelCheck2(id: string) {
      ElMessageBox.confirm("确认拒绝该陪诊师的申请吗?", "提示").then(() => {
        baseService.get(`/admin/user/assist/reject/${id}`).then((res) => {
          if (res.code === 200) {
            getList();
            ElMessage({
              message: '已拒绝陪诊师申请',
